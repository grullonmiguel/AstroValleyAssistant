...
namespace AstroValleyAssistant.Core.Services
{
    public class RealTaxDeedClient : IRealTaxDeedClient
    {
        ...
        //public async Task<List<AuctionRecord>> GetAuctionsAsync(string url, CancellationToken ct = default, IProgress<(int Current, int Total)> progress = null)
        //{
        ...
        //    // 1. Declare this OUTSIDE the loop so it persists across all pages
        //    int totalItems = 0;
        //    int currentPage = 1;
        ...
        //    while (true)
        //    {
        //        // Check if the user requested cancellation before starting a new page
        //        ct.ThrowIfCancellationRequested();

        //        long ts = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();

        //        // Logic to signal page advancement to the server's internal state
        ...
        //        // Stop if the response is empty or contains no items
        //        if (string.IsNullOrWhiteSpace(rawRetHtml) || !rawRetHtml.Contains("AITEM_")) break;

        //        // 2. Only calculate total once (on page 1), but keep the value for the whole loop
        //        if (currentPage == 1)
        //        {
        //            // This will print the first 2000 characters of the HTML to your Output window
        //            Debug.WriteLine("DEBUG HTML: " + rawRetHtml.Substring(0, Math.Min(rawRetHtml.Length, 2000)));

        //            // Pattern 1: Look for "15 Records Found" or "15 total items"
        //            var matchRecords = Regex.Match(rawRetHtml, @"(\d+)\s*(?:Records Found|total items|Items Found)", RegexOptions.IgnoreCase);

        //            // Pattern 2: Look for "Page 1 of 5"
        //            var matchPages = Regex.Match(rawRetHtml, @"Page\s+\d+\s+of\s+(\d+)", RegexOptions.IgnoreCase);

        //            if (matchRecords.Success)
        //            {
        //                int.TryParse(matchRecords.Groups[1].Value, out totalItems);
        //            }
        //            else if (matchPages.Success)
        //            {
        //                if (int.TryParse(matchPages.Groups[1].Value, out int pages))
        //                {
        //                    // Usually 10 or 20 items per page; 20 is the default for most RealAuction counties
        //                    totalItems = pages * 20;
        //                }
        //            }
        //        }

        //        // Pattern 3: Look for a hidden 'total_items' or 'max_pages' variable
        //        if (totalItems == 0)
        //        {
        //            var matchHidden = Regex.Match(rawRetHtml, @"total_items""\s*value=""(\d+)""", RegexOptions.IgnoreCase);
        //            if (matchHidden.Success)
        //            {
        //                int.TryParse(matchHidden.Groups[1].Value, out totalItems);
        //            }
        //        }
        ...
        //        if (pageRecords.Any())
        //        {
        //            // Safety break: if the first item matches the previous page, the server didn't advance
        //            if (pageRecords[0].ParcelId == lastFirstParcelId) break;

        //            allRecords.AddRange(pageRecords);
        //            lastFirstParcelId = pageRecords[0].ParcelId;

        //            // 4. This will now work because totalItems is still set from the first page!
        //            progress?.Report((allRecords.Count, totalItems));

        //            currentPage++;

        //            // Small delay to prevent rate limiting between page fetches
